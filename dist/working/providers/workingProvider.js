...
class WorkerProvider {
  ...
  async start(scriptPath, data, completionCallback) {
    ...
    this.worker_ = new Worker(path.resolve(__dirname, '../providers/workerScript.js'), data);
    if (this.eventEmitter_) this.eventEmitter_.emit('worker:start', {
      scriptPath,
      data
    });
    this.worker_.on('message', message => {
      if (message.type === 'status') {
        ...
        if (this.eventEmitter_) this.eventEmitter_.emit('worker:status', {
          status: this.status_,
          data: message.data
        });
    ...
    this.worker_.on('error', err => {
      ...
      if (this.eventEmitter_) this.eventEmitter_.emit('worker:error', {
        error: err.message
      });
      if (this.completionCallback_) {
    ...
    this.worker_.on('exit', code => {
      if (code !== 0 && this.status_ !== 'error') {
        ...
        if (this.eventEmitter_) this.eventEmitter_.emit('worker:exit:error', {
          code
        });
        if (this.completionCallback_) {
      ...
      if (this.eventEmitter_) this.eventEmitter_.emit('worker:exit', {
        code
      });
      this.worker_ = null;
    });
