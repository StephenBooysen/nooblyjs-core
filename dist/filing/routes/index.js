...
module.exports = (options, eventEmitter, filing) => {
  ...
  if (options['express-app'] && filing) {
    ...
    app.post('/api/filing/upload/:key', (req, res) => {
      ...
      filing.upload(key, value).then(() => res.status(200).send('OK')).catch(err => res.status(500).send(err.message));
    });
    app.get('/api/filing/download/:key', (req, res) => {
      const key = req.params.key;
      filing.download(key).then(value => res.status(200).json(value)).catch(err => res.status(500).send(err.message));
    });
