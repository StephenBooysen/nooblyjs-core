...
class FtpFilingProvider {
  ...
  async create(filePath, content) {
    ...
    return new Promise((resolve, reject) => {
      this.client.put(Buffer.from(content), filePath, err => {
        if (err) {
          if (this.eventEmitter_) this.eventEmitter_.emit('filing:create:error', {
            filePath,
            error: err.message
          });
          return reject(err);
        }
        if (this.eventEmitter_) this.eventEmitter_.emit('filing:create', {
          filePath,
          content
        });
        resolve();
  ...
  async read(filePath) {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.client.get(filePath, (err, stream) => {
        if (err) {
          if (this.eventEmitter_) this.eventEmitter_.emit('filing:read:error', {
            filePath,
            error: err.message
          });
          return reject(err);
        ...
        stream.on('end', () => {
          if (this.eventEmitter_) this.eventEmitter_.emit('filing:read', {
            filePath,
            content: data
          });
          resolve(data);
  ...
  async delete(filePath) {
    ...
    return new Promise((resolve, reject) => {
      this.client.delete(filePath, err => {
        if (err) {
          if (this.eventEmitter_) this.eventEmitter_.emit('filing:delete:error', {
            filePath,
            error: err.message
          });
          return reject(err);
        }
        if (this.eventEmitter_) this.eventEmitter_.emit('filing:delete', {
          filePath
        });
        resolve();
      });
  ...
  async list(dirPath) {
    ...
    return new Promise((resolve, reject) => {
      this.client.list(dirPath, (err, list) => {
        if (err) {
          if (this.eventEmitter_) this.eventEmitter_.emit('filing:list:error', {
            dirPath,
            error: err.message
          });
          return reject(err);
        ...
        if (this.eventEmitter_) this.eventEmitter_.emit('filing:list', {
          dirPath,
          files
        });
        resolve(files);
  ...
  async update(filePath, content) {
    ...
    return new Promise((resolve, reject) => {
      this.client.put(Buffer.from(content), filePath, err => {
        if (err) {
          if (this.eventEmitter_) this.eventEmitter_.emit('filing:update:error', {
            filePath,
            error: err.message
          });
          return reject(err);
        }
        if (this.eventEmitter_) this.eventEmitter_.emit('filing:update', {
          filePath,
          content
        });
        resolve();
