...
module.exports = (options, eventEmitter, dataserve) => {
  if (options['express-app'] && dataserve) {
    ...
    app.post('/api/dataserve/put/:key', (req, res) => {
      ...
      dataserve.put(key, value).then(() => res.status(200).send('OK')).catch(err => res.status(500).send(err.message));
    });
    app.get('/api/dataserve/get/:key', (req, res) => {
      const key = req.params.key;
      dataserve.get(key).then(value => res.status(200).json(value)).catch(err => res.status(500).send(err.message));
    });
