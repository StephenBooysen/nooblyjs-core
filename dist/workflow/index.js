...
class WorkflowService {
  ...
  async runWorkflow(workflowName, data, statusCallback) {
    ...
    if (!steps) {
      ...
      if (this.eventEmitter_) this.eventEmitter_.emit('workflow:error', {
        workflowName,
        error: error.message
      });
      throw error;
    ...
    if (this.eventEmitter_) this.eventEmitter_.emit('workflow:start', {
      workflowName,
      initialData: data
    });
    for (let i = 0; i < steps.length; i++) {
      ...
      if (this.eventEmitter_) this.eventEmitter_.emit('workflow:step:start', {
        workflowName,
        stepName,
        stepPath,
        data: currentData
      ...
      try {
        ...
        if (this.eventEmitter_) this.eventEmitter_.emit('workflow:step:end', {
          workflowName,
          stepName,
          stepPath,
          data: currentData
        ...
        if (this.eventEmitter_) this.eventEmitter_.emit('workflow:step:error', {
          workflowName,
          stepName,
          stepPath,
          error: error.message
    ...
    if (this.eventEmitter_) this.eventEmitter_.emit('workflow:complete', {
      workflowName,
      finalData: currentData
    });
  }
