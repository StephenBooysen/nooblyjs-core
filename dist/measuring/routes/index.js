...
module.exports = (options, eventEmitter, measuring) => {
  if (options['express-app'] && measuring) {
    ...
    app.post('/api/measuring/add', (req, res) => {
      ...
      if (metric && value) {
        measuring.add(metric, value).then(() => res.status(200).send('OK')).catch(err => res.status(500).send(err.message));
      } else {
        res.status(400).send('Bad Request: Missing metric or value');
      }
    });
    app.get('/api/measuring/list/:metric/:datestart/:dateend', (req, res) => {
      measuring.list(req.params.metric, new Date(req.params.datestart), new Date(req.params.dateend)).then(value => res.status(200).json(value)).catch(err => res.status(500).send(err.message));
    });
    app.get('/api/measuring/total/:metric/:datestart/:dateend', (req, res) => {
      measuring.total(req.params.metric, new Date(req.params.datestart), new Date(req.params.dateend)).then(value => res.status(200).json(value)).catch(err => res.status(500).send(err.message));
    });
    app.get('/api/measuring/average/:metric/:datestart/:dateend', (req, res) => {
      measuring.average(req.params.metric, new Date(req.params.datestart), new Date(req.params.dateend)).then(value => res.status(200).json(value)).catch(err => res.status(500).send(err.message));
    });
