...
module.exports = (options, eventEmitter, queue) => {
  if (options['express-app'] && queue) {
    ...
    app.post('/api/queueing/enqueue', (req, res) => {
      ...
      if (task) {
        queue.enqueue(task).then(() => res.status(200).send('OK')).catch(err => res.status(500).send(err.message));
      } else {
        res.status(400).send('Bad Request: Missing task');
      }
    });
    app.get('/api/queueing/dequeue', (req, res) => {
      queue.dequeue().then(task => res.status(200).json(task)).catch(err => res.status(500).send(err.message));
    });
