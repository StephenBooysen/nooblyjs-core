...
module.exports = (options, eventEmitter, notifier) => {
  if (options['express-app'] && notifier) {
    ...
    app.post('/api/notifying/topic', (req, res) => {
      ...
      if (topic) {
        notifier.createTopic(topic).then(() => res.status(200).send('OK')).catch(err => res.status(500).send(err.message));
      } else {
    ...
    app.post('/api/notifying/subscribe/topic/:topic', (req, res) => {
      ...
      if (topic && callbackUrl) {
        notifier.subscribe(topic, callbackUrl).then(() => res.status(200).send('OK')).catch(err => res.status(500).send(err.message));
      } else {
    ...
    app.post('/api/notifying/unsubscribe/topic/:topic', (req, res) => {
      ...
      if (topic && callbackUrl) {
        notifier.unsubscribe(topic, callbackUrl).then(() => res.status(200).send('OK')).catch(err => res.status(500).send(err.message));
      } else {
        res.status(400).send('Bad Request: Missing topic or callback URL');
      }
    });
    app.post('/api/notifying/notify/topic/:topic', (req, res) => {
      ...
      if (topic && message) {
        notifier.notify(topic, message).then(() => res.status(200).send('OK')).catch(err => res.status(500).send(err.message));
      } else {
