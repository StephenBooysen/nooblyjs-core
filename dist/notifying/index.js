...
class NotificationService {
  ...
  createTopic(topicName) {
    if (!this.topics.has(topicName)) {
      ...
      if (this.eventEmitter_) this.eventEmitter_.emit('notification:createTopic', {
        topicName
      });
    }
  }
  ...
  subscribe(topicName, callback) {
    ...
    if (this.eventEmitter_) this.eventEmitter_.emit('notification:subscribe', {
      topicName
    });
  }
  ...
  unsubscribe(topicName, callback) {
    if (this.topics.has(topicName)) {
      ...
      if (unsubscribed && this.eventEmitter_) this.eventEmitter_.emit('notification:unsubscribe', {
        topicName
      });
      return unsubscribed;
    }
  ...
  notify(topicName, message) {
    if (this.topics.has(topicName)) {
      this.topics.get(topicName).forEach(callback => {
        try {
          callback(message);
          if (this.eventEmitter_) this.eventEmitter_.emit('notification:notify', {
            topicName,
            message
          });
        } catch (error) {
          console.error(`Error in notification callback for topic ${topicName}:`, error);
          if (this.eventEmitter_) this.eventEmitter_.emit('notification:notify:error', {
            topicName,
            message,
            error: error.message
          });
