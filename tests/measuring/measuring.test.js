...
describe('MeasuringService', () => {
  ...
  it('should add a measure to a metric', () => {
    ...
    expect(measuringService.metrics.get('Orders per day')[0].timestamp).toBeInstanceOf(Date);
    expect(mockEventEmitter.emit).toHaveBeenCalledWith('measuring:add', {metricName: 'Orders per day', measure: expect.any(Object)});
  });
  ...
  it('should list measures within a specified period', () => {
    ...
    expect(mockEventEmitter.emit).toHaveBeenCalledWith('measuring:list', {metricName, startDate: yesterday, endDate: now, measures});
  });
  ...
  it('should calculate the total of measures within a specified period', () => {
    ...
    expect(mockEventEmitter.emit).toHaveBeenCalledWith('measuring:total', {metricName, startDate: yesterday, endDate: now, total});
  });
  ...
  it('should calculate the average of measures within a specified period', () => {
    ...
    expect(mockEventEmitter.emit).toHaveBeenCalledWith('measuring:average', {metricName, startDate: yesterday, endDate: now, average});
  });
  ...
  it('should return 0 for total if no measures are found in the period', () => {
    ...
    expect(total).toBe(0);
    expect(mockEventEmitter.emit).toHaveBeenCalledWith('measuring:total', {metricName, startDate: now, endDate: now, total: 0});
  });
  ...
  it('should return 0 for average if no measures are found in the period', () => {
    ...
    expect(mockEventEmitter.emit).toHaveBeenCalledWith('measuring:average', {metricName, startDate: now, endDate: now, average: 0});
  });
  ...
  it('should handle non-existent metrics gracefully', () => {
    ...
    expect(average).toBe(0);
    expect(list).toEqual([]);
    expect(mockEventEmitter.emit).toHaveBeenCalledWith('measuring:total', {metricName: 'NonExistentMetric', startDate: now, endDate: now, total: 0});
    expect(mockEventEmitter.emit).toHaveBeenCalledWith('measuring:average', {metricName: 'NonExistentMetric', startDate: now, endDate: now, average: 0});
    expect(mockEventEmitter.emit).toHaveBeenCalledWith('measuring:list', {metricName: 'NonExistentMetric', startDate: now, endDate: now, measures: []});
  });
});
