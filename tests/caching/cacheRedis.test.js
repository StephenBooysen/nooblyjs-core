...
describe('CacheRedis', () => {
  ...
  beforeEach(() => {
    ...
    cache = createCache('redis', { host: '127.0.01', port: 6379}, mockEventEmitter);
  });
  ...
  it('should delete a value', async () => {
    ...
    expect(mockEventEmitter.emit).toHaveBeenCalledWith('cache:get', {key: 'key', value: null});
  });
  ...
  it('should return null for a non-existent key', async () => {
    ...
    expect(value).toBeNull();
    expect(mockEventEmitter.emit).toHaveBeenCalledWith('cache:get', {key: 'non-existent-key', value: null});
  });
});
