...
describe('CacheMemcached', () => {
  ...
  beforeEach(() => {
    ...
    cacheMemcached = new CacheMemcached({url: mockMemcachedUrl}, mockEventEmitter);
  });

  it('should initialize with a valid URL', () => {
    expect(Client.create).toHaveBeenCalledWith(mockMemcachedUrl);
    expect(cacheMemcached.client_).toEqual(require('memjs').Client.create(mockMemcachedUrl));
  });

  it('should throw an error if no URL is provided', () => {
    expect(() => new CacheMemcached({}, mockEventEmitter)).toThrow('Memcached connection URL is required.');
    expect(() => new CacheMemcached(undefined, mockEventEmitter)).toThrow('Memcached connection URL is required.');
  });

  it('should call client.set when put is called with a string value', async () => {
    await cacheMemcached.put('testKey', 'testValue');
    expect(mockSet).toHaveBeenCalledWith('testKey', 'testValue', {expires: 0});
    expect(mockEventEmitter.emit).toHaveBeenCalledWith('cache:put', {key: 'testKey', value: 'testValue', ttl: undefined});
  });

  it('should call client.set when put is called with an object value', async () => {
    const obj = {a: 1, b: 'hello'};
    await cacheMemcached.put('testObjectKey', obj);
    expect(mockSet).toHaveBeenCalledWith('testObjectKey', JSON.stringify(obj), {expires: 0});
    expect(mockEventEmitter.emit).toHaveBeenCalledWith('cache:put', {key: 'testObjectKey', value: obj, ttl: undefined});
  });

  it('should call client.set with TTL when put is called with ttl', async () => {
    await cacheMemcached.put('testKeyWithTtl', 'testValue', 60);
    expect(mockSet).toHaveBeenCalledWith('testKeyWithTtl', 'testValue', {expires: 60});
    expect(mockEventEmitter.emit).toHaveBeenCalledWith('cache:put', {key: 'testKeyWithTtl', value: 'testValue', ttl: 60});
  });
  ...
  it('should call client.get and return string value', async () => {
    ...
    expect(mockEventEmitter.emit).toHaveBeenCalledWith('cache:get', {key: 'testKey', value: 'retrievedValue'});
  });
  ...
  it('should call client.get and return parsed object value', async () => {
    ...
    expect(mockEventEmitter.emit).toHaveBeenCalledWith('cache:get', {key: 'testObjectKey', value: obj});
  });
  ...
  it('should return null if key not found', async () => {
    ...
    expect(mockEventEmitter.emit).toHaveBeenCalledWith('cache:get', {key: 'nonExistentKey', value: null});
  });
  ...
  it('should call client.delete when delete is called', async () => {
    ...
    expect(mockEventEmitter.emit).toHaveBeenCalledWith('cache:delete', {key: 'keyToDelete'});
  });
});
