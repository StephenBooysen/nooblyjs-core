...
async function runWorkflowLoadTest(iterations) {
  ...
  await fs.writeFile(step1Path, 'module.exports = (data) => { data.step1 = true; return data; };');
  await fs.writeFile(step2Path, 'module.exports = (data) => { data.step2 = true; return data; };');
  ...
  for (let i = 0; i < iterations; i++) {
    ...
    await workflowService.runWorkflow('loadTestWorkflow', initialData, () => {});
    if (i % 10 === 0) {
  ...
  console.log(`Workflow Load Test Completed: ${iterations} operations in ${duration} ms.`);
