...
async function runCachingLoadTest(iterations, cacheType = 'memory', options = {}) {
  const eventEmitter = new EventEmitter();
  const cache = createCache(cacheType, options, eventEmitter);
  const startTime = Date.now();
  console.log(`Starting Caching Load Test (${cacheType} cache) for ${iterations} iterations...`);
  ...
  console.log(`Caching Load Test Completed: ${iterations} operations in ${duration} ms.`);
