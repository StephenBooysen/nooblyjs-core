...
async function runLoggingLoadTest(iterations, loggerType = 'console', options = {}) {
  const eventEmitter = new EventEmitter();
  const logger = createLogger(loggerType, options, eventEmitter);
  const logFilePath = path.join(__dirname, 'load_test.log');

  const startTime = Date.now();
  console.log(`Starting Logging Load Test (${loggerType} logger) for ${iterations} iterations...`);
  ...
  console.log(`Logging Load Test Completed: ${iterations} operations in ${duration} ms.`);
