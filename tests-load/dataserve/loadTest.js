...
async function runDataserveLoadTest(iterations, dataserveType = 'memory', options = {}) {
  const eventEmitter = new EventEmitter();
  const dataserve = createDataserveService(dataserveType, options, eventEmitter);
  const containerName = 'loadTestContainer';

  const startTime = Date.now();
  console.log(`Starting Dataserve Load Test (${dataserveType} dataserve) for ${iterations} iterations...`);
  ...
  console.log(`Dataserve Load Test Completed: ${iterations} operations in ${duration} ms.`);
