...
async function runFilingLoadTest(iterations, filingType = 'local', options = {}) {
  ...
  console.log(`Starting Filing Load Test (${filingType} filing) for ${iterations} iterations...`);
  ...
  for (let i = 0; i < iterations; i++) {
    const filePath = filingType === 'local' ? path.join(testDir, `file-${i}.txt`) : `file-${i}.txt`;
  ...
  console.log(`Filing Load Test Completed: ${iterations} operations in ${duration} ms.`);
