...
class SearchService {
  ...
  add(key, jsonObject) {
    if (this.data.has(key)) {
      if (this.eventEmitter_) this.eventEmitter_.emit('search:add:error', {key, jsonObject, error: 'Key already exists.'});
      return false;
    }
    this.data.set(key, jsonObject);
    if (this.eventEmitter_) this.eventEmitter_.emit('search:add', {key, jsonObject});
    return true;
  }
  ...
  remove(key) {
    ...
    if (removed && this.eventEmitter_) this.eventEmitter_.emit('search:remove', {key});
    return removed;
  }
  ...
  search(searchTerm) {
    const results = [];
    if (!searchTerm) {
      if (this.eventEmitter_) this.eventEmitter_.emit('search:search', {searchTerm, results});
      return results;
    }
