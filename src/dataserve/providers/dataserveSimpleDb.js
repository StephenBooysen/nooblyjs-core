...
class SimpleDbDataRingProvider {
  ...
  async createContainer(domainName) {
    ...
    if (this.eventEmitter_) this.eventEmitter_.emit('dataserve:createContainer', {domainName});
  }
  ...
  async add(domainName, jsonObject) {
    ...
    if (this.eventEmitter_) this.eventEmitter_.emit('dataserve:add', {domainName, itemName, jsonObject});
    return itemName;
  }
  ...
  async remove(domainName, objectKey) {
    ...
    try {
      ...
      if (this.eventEmitter_) this.eventEmitter_.emit('dataserve:remove', {domainName, objectKey});
      return true;
    } catch (error) {
      ...
      console.warn(`Error deleting item ${objectKey} from domain ${domainName}: ${error.message}`);
      return false;
    }
  }
  ...
  async find(domainName, searchTerm) {
    ...
    if (!data.Items) {
      if (this.eventEmitter_) this.eventEmitter_.emit('dataserve:find', {domainName, searchTerm, results: []});
      return [];
    }
